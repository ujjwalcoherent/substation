  ...
  const extractSegmentsFromSource = async (sourceData: RawJsonData, sourceName: string) => {
    ...
    for (let geoIdx = 0; geoIdx < geographies.length; geoIdx++) {
      ...
      if (sourceData[geography]?.[segmentType]) {
        ...
        // For "By Region" segment type, build hierarchy using regions as roots (not geography)
        if (isRegionSegmentType && geography !== 'Global') {
          // Skip adding geography as parent - we'll add regions directly instead
          ...
          if (regionData && typeof regionData === 'object') {
            ...
            // For "By Region", the directChildren ARE the regions (Asia Pacific, Middle East, etc.)
            ...
            console.log(`👶 Filtered directChildren (these are regions):`, directChildren)

            // Build hierarchy for each region (not using geography as parent)
            directChildren.forEach(region => {
              // Add region to segmentItems if not already there
              if (!segmentItems.includes(region)) {
                segmentItems.push(region)
                console.log(`  ✅ Added region '${region}' to segmentItems`)
              }

              // Build hierarchy for this region (only if not already built)
              if (!hierarchy[region]) {
                hierarchy[region] = []
              } else if (hierarchy[region].length > 0) {
                console.log(`  ⏭️ Skipping '${region}' - hierarchy already built:`, hierarchy[region])
                return
              }

              // Get countries/sub-regions under this region
              const regionChildData = regionData[region]
              if (regionChildData && typeof regionChildData === 'object') {
                const regionChildKeys = Object.keys(regionChildData).filter(key => {
                  return !/^\d{4}$/.test(key) && key !== 'CAGR' && key !== '_aggregated' && key !== '_level'
                })

                console.log(`  📂 Found children for '${region}':`, regionChildKeys)

                regionChildKeys.forEach(child => {
                  if (!hierarchy[region].includes(child)) {
                    hierarchy[region].push(child)
                    console.log(`    ➕ Added '${child}' to hierarchy['${region}']`)
                  }

                  // Also check if this child has its own children (sub-regions)
                  const childData = regionChildData[child]
                ...
                console.log(`  ✅ Final hierarchy['${region}']:`, hierarchy[region])
              }
            })
          } else {
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    const metadata: Metadata = {
      market_name: 'Online Substation Monitoring System',
